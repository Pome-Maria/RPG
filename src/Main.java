...
public class Main {
    ...
    private static void command(String string) throws IOException {
        ...
        if (player == null) {
            ...
            System.out.println(String.format("Спасти наш мир от драконов вызвался %s! Да здравствует герой!",
                    player.getName()));
    ...
    private static void commitSell() throws IOException {
        ...
        if (player != null) {
            Merchant trader = new Merchant("Торговец", 50, 10);
...
// после покупки зелья
            if (player.getGold() >= price) {
                ...
                System.out.println("Обмен на деньги завершен! Желаете продолжить? да/нет");
            ...
            if (player.getGold() < price) {
                ...
                System.out.println("У вас нет на это денег." +
    ...
    private static void commitFight() {
        ...
        battleScene.fight(player, createMonster(), new FightCallback() {
            ...
            public void fightWin() {
                System.out.println(String.format("Победа! Теперь у вас %d опыта и %d золота, осталось %d единиц здоровья.",
                        player.getName(), player.getExperience(), player.getGold(), player.getHealth()));
                System.out.println("Желаете продолжить путешествие или вернуться в город? (да/нет)");
                try {
                    command(br.readLine());
                } catch (IOException e) {
                    e.printStackTrace();
    ...
    private static FantasyCharacter createMonster() {
        ...
        if (random % 2 == 0) return new Orc(
                "Орк", 50, 10, 10, 100, 20);
        else return new EarthenHomunculus(
                "Земляной гомункул", 25, 20, 20, 100, 10);
    }
